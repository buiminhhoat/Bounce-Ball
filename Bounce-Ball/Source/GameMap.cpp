...
void GameMap::loadMap(const char* nameFileMap) {
	...
	gameMap = new Map;
	input >> gameMap->row >> gameMap->col;

	gameMap->block = (int**)malloc(sizeof(int*) * gameMap->row);
	for (int i = 0; i < gameMap->row; i++) {
		gameMap->block[i] = (int*)malloc(sizeof(int) * gameMap->col);
	}
...
void GameMap::loadIMGBlock(SDL_Renderer* screen) {
	...
	FILE* f = NULL;
	type_block[GROUND_BLOCK].loadImage("Resources/Image/block/GROUND_BLOCK.png", screen);
}
